...
int psp2link_accept_pkt(int sock, char *buf, int len, int pkt_type)
{
    ...
    if (length < sizeof(psp2link_pkt_hdr)) {
        debugNetPrintf(ERROR,"did not receive a full header!!!! "
                  "Fix this! (%d)\n", length);
    }
    ...
    if (hlen > len) {
        debugNetPrintf(ERROR,"psp2link_accept_pkt: hdr->len is too large!! "
    ...
    if (length < (hlen - sizeof(psp2link_pkt_hdr))) {
        debugNetPrintf(ERROR,"psp2link_accept_pkt: Did not receive full packet!!! "
                  "Fix this! (%d)\n", length);
    }
...
int psp2LinkIoClose(SceUID fd)
{
    ...
    if(!psp2link_accept_pkt(psp2link_fileio_sock, (char *)closerly, 
                       sizeof(psp2link_pkt_file_rly), PSP2LINK_CLOSE_RLY)) {
...
int psp2LinkIoRead(SceUID fd, void *data, SceSize size)
{
    ...
    if(!psp2link_accept_pkt(psp2link_fileio_sock, (char *)readrly, 
                       sizeof(psp2link_pkt_read_rly), PSP2LINK_READ_RLY)) {
        debugNetPrintf(ERROR,"psp2link_read_file: "
                  "did not receive PSP2LINK_READ_RLY\n");
        return -1;
    }

    nbytes = sceNetNtohl(readrly->nbytes);
    debugNetPrintf(DEBUG,"psp2link_read_file: Reply said there's %d bytes to read "
              "(wanted %d)\n", nbytes, size);
...
int psp2LinkIoWrite(SceUID fd, const void *data, SceSize size)
{
	...
	/* Send the write() data.  */
        ...
        memcpy(&send_packet[sizeof(psp2link_pkt_write_req)], &data[writtenbytes],
               nbytes);
        ...
        // Get reply
        if(!psp2link_accept_pkt(psp2link_fileio_sock, (char *)writerly, 
                           sizeof(psp2link_pkt_file_rly), PSP2LINK_WRITE_RLY)) {
            debugNetPrintf(ERROR,"psp2link_write_file: "
                      "did not receive PSP2LINK_WRITE_RLY\n");
            return -1;
        }
...
int psp2LinkIoRemove(const char *file)
{
    ...
    if (!psp2link_accept_pkt(psp2link_fileio_sock, recv_packet, 
                        sizeof(psp2link_pkt_file_rly), PSP2LINK_REMOVE_RLY)) {
...
int psp2LinkIoMkdir(const char *dirname, SceMode mode)
{
    ...
    if (!psp2link_accept_pkt(psp2link_fileio_sock, recv_packet, 
                        sizeof(psp2link_pkt_file_rly), PSP2LINK_MKDIR_RLY)) {
...
int psp2LinkIoRmdir(const char *dirname)
{
    ...
    if (!psp2link_accept_pkt(psp2link_fileio_sock, recv_packet, 
                        sizeof(psp2link_pkt_file_rly), PSP2LINK_RMDIR_RLY)) {
...
int psp2LinkIoDopen(const char *dirname)
{
    ...
    if (!psp2link_accept_pkt(psp2link_fileio_sock, recv_packet, 
                        sizeof(psp2link_pkt_file_rly), PSP2LINK_OPENDIR_RLY)) {
...
int psp2LinkIoDread(SceUID fd, SceIoDirent *dir)
{
    ...
    if (!psp2link_accept_pkt(psp2link_fileio_sock, recv_packet, 
                        sizeof(psp2link_pkt_dread_rly), PSP2LINK_READDIR_RLY)) {
...
int psp2LinkIoDclose(SceUID fd)	
{
    ...
    if(!psp2link_accept_pkt(psp2link_fileio_sock, (char *)closerly, 
                       sizeof(psp2link_pkt_file_rly), PSP2LINK_CLOSEDIR_RLY)) {
